...
HAL_StatusTypeDef SSD1306_Update(SSD1306_Handle *dev)
{
    ...
    uint8_t tx_buf[SSD1306_BUF_SIZE + 1];
...
static void eeprom_wait_ready(I2C_HandleTypeDef *hi2c)
{
    /* Acknowledge polling: keep trying until the EEPROM responds */
    while (HAL_I2C_IsDeviceReady(hi2c, EEPROM_ADDR, 10, 100) != HAL_OK)
    {
        /* EEPROM is still busy with the previous write cycle */
    }
}
