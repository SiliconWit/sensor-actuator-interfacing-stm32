...
5. **Launch STM32CubeMX** to verify. On first run it may download firmware packages for your target MCU family (STM32F1 in our case). This requires an internet connection.
</Steps>
...
4. After configuring all peripherals, go to the **Project Manager** tab. Set the project name to `ProximityAlarm`, choose a project location, and set **Toolchain / IDE** to **STM32CubeIDE**. Click **GENERATE CODE**.
</Steps>

**In STM32CubeIDE:**

<Steps>
1. **Launch STM32CubeIDE** and select your workspace folder.

2. **File > Open Projects from File System**. Click **Directory**, browse to the project folder that CubeMX generated (e.g., `~/STM32Projects/ProximityAlarm`), and click **Finish**.

3. The project appears in the Project Explorer. You can now edit `main.c`, build (hammer icon), and flash/debug (green play button or bug icon).
</Steps>
