...
void parse_rmc(char *sentence) {
    char *time_str = nmea_field(sentence, 1);
    char *status   = nmea_field(sentence, 2);
    char *lat      = nmea_field(sentence, 3);
    char *lat_dir  = nmea_field(sentence, 4);
    char *lon      = nmea_field(sentence, 5);
    char *lon_dir  = nmea_field(sentence, 6);
    char *spd      = nmea_field(sentence, 7);
    char *crs      = nmea_field(sentence, 8);
    char *date_str = nmea_field(sentence, 9);
    ...
    if (gps.fix_valid) {
        /* nmea_field uses a static buffer, so copy fields before calling again */
        char lat_buf[20], lat_d[4], lon_buf[20], lon_d[4];
        strncpy(lat_buf, nmea_field(sentence, 3), 19);
        strncpy(lat_d,   nmea_field(sentence, 4), 3);
        strncpy(lon_buf, nmea_field(sentence, 5), 19);
        strncpy(lon_d,   nmea_field(sentence, 6), 3);
